...
void runSelector(
	TString runNumber = "30496",
	TString myPath = "/sciclone/gluex10/RunPeriod-2017-01/analysis/ver35/tree_pi0pi0pippim__B4/merged/",
	TString myOption = "")
{
	...
	if (myPath.Contains("tree_thrown"))
	...
	if (!files)
	{
		std::cerr << "No files found in directory: " << sampleDir.Data() << "\n";
		return 1;
	}
	...
	DPROOFLiteManager::Process_Chain(
		chain,
		"DSelector_omegapi_all.C++",
		Proof_Nthreads,
		Form("hist_pomegapi_omega3pi_%s.acc.root", runNumber.Data()),
		Form("tree_pomegapi_omega3pi_%s.acc.root", runNumber.Data()),
		Form("%s DEFAULTFLATOFF", myOption.Data()));
